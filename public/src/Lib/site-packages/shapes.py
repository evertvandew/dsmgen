...
def styled(default_style):
    """ A decorator to add a styling system to a dataclass.
        This system allows inheritance and setting a default style for a type of element.
    """
    def decorator(cls):
        # Give the cls a 'styling' element, compatible with a dataclass.
        cls.__annotations__['styling'] = dict
        cls.styling = field(default_factory=dict)
        # Create or update the dictionary with default styles
        ds = {}
        ds.update(getattr(cls, 'default_style', {}))
        ds.update(default_style)
        cls.default_style = ds
        # Give cls a function to retrieve a styling element
        def getStyle(self, key):
            assert key in self.default_style, f"Trying to retrieve unknown styling element {key}"
            return self.styling.get(key, self.default_style.get(key, ''))
        cls.getStyle = getStyle
        return cls
    return decorator
...
def renderText(text, x, y, width, height, style):
    ...
    rendered = [svg.text(line, x=xpos, y=int(ypos+lineheight*(i+1)), text_anchor=anchor, font_size=fontsize, font_family=style.getStyle('font'))
...
class Rect(BasicShape):
    ...
    def getShape(cls, details):
        return svg.rect(x=details.x, y=details.y, width=details.width, height=details.height,
                        stroke_width=details.getStyle('bordersize'),
                        stroke=details.getStyle('bordercolor'),
                        fill=details.getStyle('blockcolor'),
                        ry=details.getStyle('cornerradius'))
    @classmethod
    def updateShape(cls, shape, details):
...
class Circle(BasicShape):
    ...
    @classmethod
    def getShape(cls, details):
        return svg.circle(cx=details.x, cy=details.y, r=(details.width+details.height)//4,
                        stroke_width=details.getStyle('bordersize'),
                        stroke=details.getStyle('bordercolor'),
                        fill=details.getStyle('blockcolor'))
    @classmethod
    def updateShape(cls, shape, details):
        ...
        shape['cx'], shape['cy'] = details.x, details.y

class Ellipse(BasicShape):
    style_items = {}
    @classmethod
    def getShape(cls, details):
        return svg.circle(cx=details.x, cy=details.y, rx=details.width//2, ry=details.height//2,
                        stroke_width=details.getStyle('bordersize'),
                        stroke=details.getStyle('bordercolor'),
                        fill=details.getStyle('blockcolor'))
    @classmethod
...
class Note(BasicShape):
    ...
    def getPoints(cls, details):
        ...
        f = int(details.getStyle('fold_size'))
        return [(x+a,y+b) for a, b in [(0,0), (w-f,0), (w,f), (w-f,f), (w-f,0), (w,f), (w,h), (0,h)]]
    @classmethod
    def getShape(cls, details):
        outline = svg.polygon(points=' '.join(f'{x},{y}' for x, y in cls.getPoints(details)), fill=details.getStyle('blockcolor'), stroke=details.getStyle('bordercolor'), stroke_width=details.getStyle('bordersize'))
        return outline
    @classmethod
    def updateShape(cls, shape, details):
...
class Component(BasicShape):
    ...
    def getShape(cls, details):
        ...
        rw, rh, rp = [int(details.getStyle(i)) for i in 'ringwidth ringheight ringpos'.split()]
        rp = rp * details.height // 100
        g <= svg.rect(x=details.x-rw//2,
                      y=details.y+rp,
                      width=details.getStyle('ringwidth'), 
                      height=details.getStyle('ringheight'),
                      stroke_width=details.getStyle('bordersize'),
                      stroke=details.getStyle('bordercolor'),
                      fill=details.getStyle('blockcolor'))
        g <= svg.rect(x=details.x-rw//2,
...
class ClosedCircle(Circle):
    ...
    def getShape(cls, details):
        ...
        shape['fill'] = details.getStyle('bordercolor')
        return shape
...
class RingedClosedCircle(BasicShape):
    ...
    def getShape(cls, details):
        ...
        ball['r'] = (details.width + details.height) // 4 - int(details.getStyle('space'))
    ...
    def updateShape(cls, shape, details):
        ...
        shape.children[0]['r'] = (details.width + details.height) // 4 - int(details.getStyle('space'))

class Bar(Rect):
    style_items = {'blockcolor': 'black', 'cornerradius': '0'}
    @classmethod
    def getShape(cls, details):
        return svg.rect(x=details.x, y=details.y, width=details.width, height=details.height,
                        stroke_width=details.getStyle('bordersize'),
                        stroke=details.getStyle('blockcolor'),
                        fill=details.getStyle('blockcolor'),
                        ry=details.getStyle('cornerradius'))
...
class Hexagon(Note):
    ...
    @classmethod
    def getPoints(cls, details):
        bump = int(details.getStyle('bump'))
        return [(details.x+a, details.y+b) for a, b in [
            (0, 0),
            (details.width, 0),
...
class Box:
    ...
    def getPoints(cls, details):
        return [(details.x + a, details.y + b) for a, b in [
            ...
            (0, details.height),
            (0, 0),
            (details.getStyle('offset'), -details.getStyle('offset')),
            (details.width+details.getStyle('offset'), -details.getStyle('offset')),
            (details.width, 0),
            (details.width + details.getStyle('offset'), -details.getStyle('offset')),
            (details.width + details.getStyle('offset'), -details.getStyle('offset')+details.height),
            (details.width, details.height)
        ]]
...
class Drum(BasicShape):
    ...
    def getPath(cls, details):
        ...
        x2, y2 = x1+w, y1+h
        ch = int(details.getStyle('curve_height')) * w // 100
...
class Stickman(Drum):
    ...
    @classmethod
    def getPath(cls, details):
        proportions = [int(i) for i in details.getStyle('proportions').split()]
        x1 = details.x
...
class Shape:
    ...
    default_style = dict(blockcolor='white', bordercolor='black', bordersize='2')
    ...
    def getStyle(self, key):
        ...
        if key in self.shape_descriptor.style_items:
            return self.shape_descriptor.style_items[key]
...
@dataclass
@styled(default_style=dict(linecolor='black', linewidth='2', endmarker='arrow', startmarker='arrow'))
class Relationship:
    start: Point
    finish: Point
