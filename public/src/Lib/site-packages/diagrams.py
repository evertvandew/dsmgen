...
class DiagramConfiguration:
    ...
    def get_allowed_connections(self, blocka_cls, blockb_cls) -> List[type]:
        return self.connections_from[blocka_cls].get(blockb_cls, [])
...
class Diagram(OwnerInterface):
    ...
    def __init__(self, config: DiagramConfiguration, widgets, api: DataStore=None, diagram_id=None):
        ## TODO: removed repr_lu={}, allowed_connections={}
        self.selection = None
        ...
        api and api.subscribe('*/*Representation/*', self, representationAction)
    ...
    def load_diagram(self):
        self.api and self.api.get_diagram_data(self.diagram_id, self.mass_update)
    ...
    def deleteConnection(self, connection):
        if connection in self.connections:
            self.api and self.api.delete(connection)
            connection.delete()
            self.connections.remove(connection)

    def deleteBlock(self, block):
        self.api and self.api.delete(block)
        block.delete()
        ...
        for c in to_remove:
            ...
            self.api and self.api.delete(c)
    ...
    def connect_specific(self, a, b, cls):
        ...
        self.api and self.api.add(connection)
    ...
    def mouseDownChild(self, widget, ev):
        ...
        def update_func(new_data):
            ...
            self.api and self.api.update(widget)
        ...
        self.api and self.api.trigger_event('shape_selected', widget, **event_detail)
    ...
    def mouseDownConnection(self, connection, ev):
        ...
        def update_func(new_data):
            ...
            # Inform the datastore of any change
            self.api and self.api.update(connection)
        ...
        self.api and self.api.trigger_event('shape_selected', connection, **event_detail)
        self.canvas.dispatchEvent(window.CustomEvent.new("shape_selected", {
            "bubbles":True,
            "detail": event_detail
        }))
    ...
    def onDrop(self, ev):
        ...
        if self.api:
            block = self.api.create_representation(block_cls.__name__, data['Id'], drop_details)
            if not block:
                return
    ...
    def updateElement(self, element):
        if is_dataclass(element):
            self.api and self.api.update(element)
...
@dataclass
class PortLabel(Block):
    Id: shapes.HIDDEN = 0
    block: shapes.HIDDEN = None
    parent: shapes.HIDDEN = None

    note_type = shapes.BasicShape.getDescriptor("label")
    logical_class = None
...
class BlockCreateWidget:
    ...
    def onMouseDown(self, ev, index, representation):
        ...
        if not instance.logical_class:
            instance.logical_class = representation.logical_class
        diagram.api.add(instance)
...
def load_diagram(diagram_id, diagram_cls, config: DiagramConfiguration, api, canvas):
    diagram = diagram_cls(config, [BlockCreateWidget, EditingModeWidget], api, diagram_id)
