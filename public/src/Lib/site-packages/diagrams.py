try:
    from browser import document, alert, svg, console, window, bind
except:
    document = {}
    alert = None
    class svg:
...
class Diagram(OwnerInterface):
    ...
    def __init__(self, widgets, api: DiagramApi=None, diagram_id=None, repr_lu={}):
        self.selection = None
    ...
    def rerouteConnections(self, widget):
        if isinstance(widget, Relationship):
            ...
            for c in self.getConnectionsToShape(widget):
                ...
                self.api and self.api.update(c)
    ...
    def mouseDownChild(self, widget, ev):
        ...
        #console.log(f'Serializing: {widget}')
    ...
    def onDrop(self, ev):
        ...
        loc = getMousePos(ev)
        console.log(f'Got drop data: {data} @ {loc}')
        ...
        console.log(f'blc: {block_cls}, {[f.name for f in fields(block_cls)]}')
...
class Block(Shape):
    ...
    def getDefaultStyle(cls):
        style = cls.note_type.getDefaultStyle()
...
def load_diagram(diagram_id, diagram_cls, api, canvas, repr_lu):
    diagram = diagram_cls([BlockCreateWidget], api, diagram_id, repr_lu)
    diagrams.append(diagram)
    diagram.bind(canvas)
