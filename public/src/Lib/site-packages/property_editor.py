...
def isEditable(field):
    if field.type is diagrams.HIDDEN:
        return False
    return issubclass(field.type, enum.IntEnum) or field.type==longstr or (isinstance(field.type, Hashable) and field.type in type2HtmlInput)
...
def getInputForField(o, field):
    ...
    if field.type == longstr:
        ...
        input = html.TEXTAREA(id=f"edit_{field.name}", name=field.name, value=value)
...
def dataClassEditorForm(o, objecttype, default=None, update=None):
    ...
    console.log(f'Editor for {o}, {objecttype}')
...
def getFormValues(form, objecttype):
    ...
    editable_fields = [f for f in fields(objecttype) if f.name not in ['x', 'y', 'height', 'width', 'styling'] and isEditable(f)]
    update_data = {}
    for field in editable_fields:
    ...
    if isStylable(objecttype):
        style_keys = objecttype.getStyleKeys()
        new_style = {key: document[f'styling_{key}'].value for key in style_keys}
