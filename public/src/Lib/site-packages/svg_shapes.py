...
class Square(BasicShape):
    ...
    @classmethod
    def getShape(cls, details):
        return svg.rect(x=details.x, y=details.y,
                        width=(details.width+details.height)//2,
                        height=(details.width+details.height)//2,
                        stroke_width=cls.getStyle('bordersize', details),
                        stroke=cls.getStyle('bordercolor', details),
                        fill=cls.getStyle('blockcolor', details))
    @classmethod
    def updateShape(cls, shape, details):
        shape['width'] = (details.width + details.height) // 2
        shape['height'] = (details.width + details.height) // 2
        shape['x'], shape['y'] = details.x, details.y
        stroke_width = cls.getStyle('bordersize', details)
        stroke = cls.getStyle('bordercolor', details)
        fill = cls.getStyle('blockcolor', details)
...
# Only the start is given, the end variant is generated from this, named "start<name>" and "end<name>"
# Also open and closed variants are generated. Open ones are named "startopen<name>" or "endopen<name>"

line_endings = {
    'arrow': "10 0, 10 7, 0 3.5",
    'triangle': "0 0, 10 5, 0 10, 0 0",
    'diamond': "0 3.5, 10 0, 20 3.5, 10 7, 0 3.5"
}
