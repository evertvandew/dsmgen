            ...
            if response.status >= 200 and response.status < 300:
                ...
                for d in response.json:
                    if 'block_cls' in d:
                        ...
                        d['waypoints'] = load_waypoints(d['routing'])
                ...
                # detecting changes.
                cp_lu = {cp.Id: cp for cp in representations if cp.repr_category() in ['block', 'port']}
                ...
                for d in [r for r in representations if r.repr_category() == 'relationship']:
                    ...
                    if d.start not in self.cache[Collection.block_repr] or \
                            d.finish not in self.cache[Collection.block_repr]:
