...
class Collection(Enum):
    ...
    def representations(cls):
        return [cls.block_repr, cls.relation_repr, cls.port_repr]
    ...
    def oppose(cls, c):
        ...
        return {
            ...
            cls.port: cls.port_repr,
            cls.port_repr: cls.port
        }[c]
...
class DataConfiguration:
    ...
    def get_collection(self, name):
        ...
        if name in self.block_entities:
            return Collection.block
        if name in self.port_entities:
            return Collection.port
        if name in self.relation_entities:
            return Collection.relation
        if name in self.block_representations or name in self.block_entities:
            return Collection.block_repr
        if name in self.relation_representations:
            return Collection.relation_repr
        if name in self.port_representations:
            return Collection.port_repr
...
class DataStore(EventDispatcher):
    def __init__(self, configuration: DataConfiguration):
        ...
        self.repr_collection_urls = {
            ...
            Collection.port_repr: '_PortRepresentation',
        }
        ...
        if collection in Collection.representations():
            ...
            if not model.Id:
                ...
                if collection == Collection.block_repr:
                    ...
                    repr['block'] = model.Id
                elif collection == Collection.port_repr:
                    record.port = model.Id
                    repr['port'] = model.Id
                else:
            ...
            if collection == Collection.block_repr and hasattr(record, 'ports'):
                ...
                if orig_ports != new_ports:
                    ...
                    for i in deleted:
                        ...
                        self.delete(self.cache[Collection.port][p.port])
                    for i in added:
                        p = lu_new[i]
    ...
    def get_diagram_data(self, diagram_id, cb: Callable):
        ...
        def on_data(response):
            if response.status >= 200 and response.status < 300:
                ...
                # First handle all the "blocks" in the diagram
                ...
                for d in [r for r in response.json if 'rel_cls' in r]:
                    ...
                    entity = dc_from_dict(cls, d)
                    records.append(entity)

                # Handle the ports
                for d in [r for r in response.json if 'port_cls' in r]:
                    # Ports are not returned in the list of items, but as part of blocks.
                    cls = self.all_classes[d['port_cls']]
                    entity = dc_from_dict(cls, d)
                    block = entity_lu[entity.block]
                    block.ports.append(entity)
    ...
    def split_representation_item(self, collection, record) -> (Dict, Any):
        ...
        if collection == Collection.block_repr:
            ...
            model = deepcopy(self.cache[Collection.oppose(collection)].get(record.port, False)) or model_cls()
            keys = {f.name for f in fields(model_cls) if hasattr(record, f.name) and f.name != 'Id'}
            for k in keys:
                if (v := getattr(record, k)) != getattr(model, k):
                    setattr(model, k, v)
            block_repr = self.cache[Collection.block_repr][record.block]
            model.parent = block_repr.block
            repr = dict(
                diagram=record.diagram,
                block=record.block,
                port=record.port,
