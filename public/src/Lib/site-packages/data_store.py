...
class DataConfiguration:
    ...
    def get_collection(self, name):
        ...
        if name in self.block_representations:
            return Collection.block_repr
        if name in self.relation_representations:
            return Collection.relation_repr
        # Hierarchy already contains block + relations, only diagrams + modelitems are new
...
class DataStore(EventDispatcher):
    ...
    def update(self, record):
        ...
        if collection in Collection.representations():
            ...
            if model != org_model:
                self.update(model)
            if any(getattr(org_repr, k) != v for k, v in repr.items() if hasattr(org_repr, k)):
    ...
    def update_cache(self, records: List[Any]|Any):
        if isinstance(records, Iterable):
            ...
            self.cache[collection][record.Id] = copy(record)

    def split_representation_item(self, collection, record) -> (Dict, Any):
        model_cls = record.logical_class
        if collection == Collection.block_repr:
            original = copy(self.cache[Collection.oppose(collection)].get(record.block, False)) or model_cls()
            keys = {f.name for f in fields(model_cls) if hasattr(record, f.name) and f.name != 'Id'}
            for k in keys:
                if (v := getattr(record, k)) != getattr(original, k):
            ...
            original = copy(self.cache[Collection.oppose(collection)].get(record.relationship, False)) or model_cls()
            keys = {f.name for f in fields(model_cls) if hasattr(record, f.name) and f.name != 'Id'}
            ...
            for k in keys:
                if (v := getattr(record, k)) != getattr(original, k):
                    setattr(original, k, v)
            original.source = record.start.block
            original.target = record.finish.block
            repr = dict(
                diagram = record.diagram,
