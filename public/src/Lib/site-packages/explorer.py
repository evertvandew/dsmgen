...
def make_explorer(holder, api: ExplorerDataApi):
    def bind_events(data_element, html_element):
        ...
        def clickfunc(ev):
            api.trigger_event('click', data_element.Id, type(data_element).__name__)
            ev.stopPropagation()
            ev.preventDefault()

        @bind(html_element, 'dblclick')
        def clickfunc(ev):
            api.trigger_event('dblclick', data_element.Id, type(data_element).__name__)
            ev.stopPropagation()
            ev.preventDefault()
