...
def make_explorer(holder, api: ExplorerDataApi):
    def bind_events(data_element, html_element):
        ...
        def clickfunc(ev):
            api.trigger_event('click', target_dbid=data_element.Id, target_type=type(data_element).__name__)
            ev.stopPropagation()
            ev.preventDefault()

        @bind(html_element, 'dblclick')
        def clickfunc(ev):
            api.trigger_event('dblclick', target_dbid=data_element.Id, target_type=type(data_element).__name__)
            ev.stopPropagation()
            ev.preventDefault()

        @bind(html_element, 'dragstart')
        def ondragstart(ev):
            console.log(f"Drag started {data_element}")
        ...
        def on_delete(ev):
            ...
            def ok(ev):
                result = api.delete_element(data_element)
                d.close()
                if result:
        ...
        def on_rename(ev):
            ...
            def on_ok(ev):
                data = d.value
                api.update_element(data_element)
    ...
    def render_hierarchy(data_list):
        ...
        for element in data_list:
            ...
            de <= html.SPAN(format_name(getattr(element, 'name', type(element).__name__)), Class=name_cls)

            d = html.DIV(de, Class=line_cls, id=element.Id,  draggable="true")
            d.value = element
