...
class DiagramApi(RestApi):
    ...
    def add_element(self, details: Any):
        ...
        if details.repr_category() == 'block':
            ...
            raise NotImplementedError(f'Category {type(details).__name__} is not supported')
    def delete_element(self, details: Any) -> bool:
        """ Called when an element is deleted.
            Returns True if the delete was successful, false if not.
        """
        ...
        ajax.delete(f'/data/_BlockRepresentation/{details.Id}', blocking=True, oncomplete=oncomplete)
        return result
    def update_element(self, details: Any):
        ...
        if details.repr_category() == 'block':
            ...
            d = asdict(details)
            block_update = {k: getattr(details, k, v)
                            for k, v in asdict(entity).items()
                            if k not in ['Id', 'parent', 'order', 'children'] and v != d[k]}
            # If any fields were updated, persist that change.
            if block_update:
                console.log(f"Updating block: {block_update} -- {asdict(entity)}")
                for k, v in block_update.items():
                    setattr(entity, k, v)
