...
* *BlockDiagram*: element that only shows up in the navigator, but double-clicking on it opens a diagram in the diagram editor.
...
* *BlockInstance*: A block that instantiates another block defined elsewhere. Used to e.g. use blocks defined in a library.
  ...
  Also usefull in UML diagrams where instances of classes are used, like the communication and sequence diagrams.
...
* Each Diagram has a specific list of Entities that can be instantiated in it. Only the Entities are limited,
  the other elements that can be added to a diagram are determined from the Entities being connected.
